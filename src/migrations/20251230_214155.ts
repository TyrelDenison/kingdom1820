...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_agent_prompts\`("id", "title", "prompt", "status", "max_credits", "last_run", "updated_at", "created_at", "_status") SELECT "id", "title", "prompt", "status", "max_credits", "last_run", "updated_at", "created_at", "_status" FROM \`agent_prompts\`;`)
  await db.run(sql`DROP TABLE \`agent_prompts\`;`)
  await db.run(sql`ALTER TABLE \`__new_agent_prompts\` RENAME TO \`agent_prompts\`;`)
  await db.run(sql`PRAGMA foreign_keys=ON;`)
  await db.run(sql`CREATE INDEX \`agent_prompts_updated_at_idx\` ON \`agent_prompts\` (\`updated_at\`);`)
